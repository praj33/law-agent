...
import { Message, GlossaryTerm, TimelineEvent } from '../types'; // Import the Message type
import FormattedResponseDisplay from './FormattedResponseDisplay'; // Import the new component
  ...
  const [messages, setMessages] = useState<Message[]>([
    {
      id: '1',
      type: 'ai',
      content: `🎯 Welcome to Law Agent - Your Advanced Legal AI Assistant!

I'm here to help you navigate complex legal matters with precision and expertise. Whether you're dealing with contract disputes, employment issues, property matters, or any other legal questions, I can provide comprehensive guidance based on Indian legal frameworks.

✨ Key Features:
• Instant analysis of legal queries
• Detailed references to BNS, IPC, and CrPC sections
• Step-by-step legal procedures
• Immediate action recommendations
• Domain-specific legal insights

Please note: This is an AI assistant and not a substitute for professional legal advice. For specific cases, always consult with a qualified attorney.

How can I assist you with your legal matter today? ⚖️`,
      timestamp: new Date()
    }
  ]);
  ...
  // State to share data between views
    ...
    try {
      ...
      const aiMessage: Message = {
        ...
        structuredData: {
          ...
          legal_route: response.legal_guidance?.legal_procedures?.join(', ') || 'Legal guidance provided',
      ...
      // Update shared data for other views
      updateSharedDataFromMessage(aiMessage);
    } catch (error: any) {
      ...
      const errorMessage: Message = {
        id: (Date.now() + 1).toString(),
        type: 'ai',
        content: `⚠️ I apologize, but I encountered an issue while processing your request. This could be due to:

• Temporary connectivity issues
• Server maintenance
• Complex query requiring more time

Please try again in a moment. If the problem persists, consider rephrasing your question or contact support.
...
Thank you for your patience! 🙏`,
  ...
  const handleFeedbackSubmit = async (messageId: string, feedbackType: 'helpful' | 'not_helpful') => {
    // Update the message in state to show feedback confirmation
    ...
    try {
      ...
      /*
      await fetch('https://law-agent-by-grok.onrender.com/api/feedback', {
        method: 'POST',
        headers: {
  ...
  // Function to extract and update shared data from AI messages
  const updateSharedDataFromMessage = (message: Message) => {
    const newData: any = {};
    
    // Extract legal domain
    if (message.structuredData?.domain) {
      newData.legalDomain = message.structuredData.domain;
      
      // Map domain to process type for 3D visualization
      const domainToProcessType: Record<string, 'criminal' | 'civil' | 'family' | 'employment'> = {
      ...
      const processType = domainToProcessType[message.structuredData.domain] || 'civil';
      newData.processType = processType;
    }
    
    // Extract jurisdiction
    if (message.structuredData?.legal_route) {
      // Simple heuristic to detect jurisdiction from legal route
      const route = message.structuredData.legal_route.toLowerCase();
      if (route.includes('india') || route.includes('bns') || route.includes('ipc')) {
        newData.jurisdiction = 'india';
      } else if (route.includes('usa') || route.includes('us')) {
        newData.jurisdiction = 'usa';
      }
    }
    
    // Extract glossary terms
    if (message.glossaryTerms && message.glossaryTerms.length > 0) {
      newData.glossaryTerms = message.glossaryTerms;
    }
    
    // Extract timeline events
    if (message.timeline && message.timeline.length > 0) {
      newData.timelineEvents = message.timeline;
    }
    
    setSharedData(prev => ({ ...prev, ...newData }));
  };

  // Function to handle visualization changes and update shared data
  const handleVisualizationChange = (view: string, data: any) => {
    if (view === 'process-step-selected') {
      setSharedData(prev => ({ ...prev, processType: data.processType }));
    } else if (view === 'jurisdiction-selected') {
      setSharedData(prev => ({ ...prev, jurisdiction: data.region }));
    }
  };
        ...
        <div className="bg-white/10 backdrop-blur-xl rounded-3xl border border-white/30 overflow-hidden shadow-2xl shadow-black/20 relative">
              ...
              <div className="flex-1 p-6 overflow-y-auto space-y-6 relative">
                  ...
                  >
                    {message.type === 'ai' && (
                      <div className="relative flex-shrink-0">
                        <div className="w-12 h-12 bg-gradient-to-r from-blue-500 to-purple-500 rounded-full flex items-center justify-center shadow-lg border-2 border-white/20">
                          <Bot className="h-6 w-6 text-white" />
                        </div>
                        <div className="absolute -bottom-1 -right-1 w-4 h-4 bg-green-400 rounded-full border-2 border-white animate-pulse" />
                      </div>
                    ...
                    <div
                      className={`max-w-4xl w-full px-6 py-4 rounded-2xl shadow-xl relative backdrop-blur-sm ${
                        message.type === 'user'
                          ? 'bg-gradient-to-r from-blue-600 to-purple-600 text-white ml-auto'
                          : 'bg-gradient-to-r from-gray-800/90 to-gray-900/90 text-gray-100 border border-white/10'
                      }`}
                    >
                      {message.type === 'ai' ? (
                        <FormattedResponseDisplay message={message} />
                      ) : (
                    ...
                    {message.type === 'ai' && !message.feedbackSubmitted && (
                      <div className="ml-16 mt-2 flex items-center space-x-2">
                        <span className="text-xs text-gray-400">Was this response helpful?</span>
                        <button
                          onClick={() => handleFeedbackSubmit(message.id, 'helpful')}
                          className="text-xs bg-green-500/20 hover:bg-green-500/30 text-green-400 px-2 py-1 rounded-lg transition-colors flex items-center"
                        >
                          <span>👍 Helpful</span>
                        </button>
                        <button
                          onClick={() => handleFeedbackSubmit(message.id, 'not_helpful')}
                          className="text-xs bg-red-500/20 hover:bg-red-500/30 text-red-400 px-2 py-1 rounded-lg transition-colors flex items-center"
                        >
                    ...
                    {message.type === 'user' && (
                      <div className="relative flex-shrink-0">
                        <div className="w-12 h-12 bg-gradient-to-r from-green-500 to-teal-500 rounded-full flex items-center justify-center shadow-lg border-2 border-white/20">
                          <User className="h-6 w-6 text-white" />
                        </div>
                        <div className="absolute -bottom-1 -right-1 w-4 h-4 bg-blue-400 rounded-full border-2 border-white animate-pulse" />
                      </div>
                    )}
                  </div>
                ))}
                ...
                {isTyping && (
                  <div className="flex items-start space-x-4 animate-fade-in">
                    ...
                    <div className="bg-gradient-to-r from-gray-800/90 to-gray-900/90 px-6 py-4 rounded-2xl border border-white/10 shadow-xl backdrop-blur-sm">
                      <div className="flex space-x-2 items-center">
                        ...
                        <span className="ml-3 text-gray-300 text-sm">Analyzing your query...</span>
                      </div>
                    </div>
                  </div>
                )}
              ...
              <div className="p-6 border-t border-white/20 bg-gradient-to-r from-gray-800/50 to-gray-900/50 backdrop-blur-sm">
                ...
                <div className="mt-4 flex flex-wrap gap-2">
                  {[
                    ...
                    "Property dispute resolution process",
                    "How to draft a legal notice?"
                  ].map((suggestion, index) => (
                    <button
                      key={index}
                      onClick={() => setInputMessage(suggestion)}
                      className="px-4 py-2 bg-gradient-to-r from-blue-600/80 to-purple-600/80 hover:from-blue-500 hover:to-purple-500 text-white rounded-full text-sm transition-all duration-200 border border-white/20 hover:border-white/40 shadow-md hover:shadow-lg"
                    >
                      {suggestion}
                    </button>
                  ))}
          ...
          {currentView === 'timeline' && (
            <div className="h-[600px] w-full">
              <div className="bg-gradient-to-br from-slate-900/90 to-blue-900/90 backdrop-blur-sm rounded-2xl border border-white/10 p-6 h-full">
                <div className="flex items-center justify-between mb-6">
                  <div>
                    <h2 className="text-2xl font-bold text-white mb-2">Legal Timeline</h2>
                    <p className="text-gray-300">Chronological view of legal procedures and events</p>
                  </div>
                  <div className="flex space-x-2">
                    <div className="px-3 py-1 bg-blue-500/20 text-blue-400 rounded-full text-sm border border-blue-500/30">
                      Timeline
                ...
                <div className="h-[500px] rounded-xl overflow-hidden border border-white/10">
                  <LegalTimeline 
                    events={sharedData.timelineEvents || []}
          ...
          {currentView === 'glossary' && (
            <div className="h-[600px] w-full">
              <div className="bg-gradient-to-br from-slate-900/90 to-blue-900/90 backdrop-blur-sm rounded-2xl border border-white/10 p-6 h-full">
                <div className="flex items-center justify-between mb-6">
                  <div>
                    <h2 className="text-2xl font-bold text-white mb-2">Legal Glossary</h2>
                    <p className="text-gray-300">Definitions of legal terms and concepts</p>
                  </div>
                  <div className="flex space-x-2">
                    <div className="px-3 py-1 bg-blue-500/20 text-blue-400 rounded-full text-sm border border-blue-500/30">
                      Glossary
                ...
                <div className="h-[500px] rounded-xl overflow-hidden border border-white/10">
                  <LegalGlossary 
                    terms={sharedData.glossaryTerms || []}
          ...
          {currentView !== 'chat' && currentView !== 'court' && currentView !== 'map' && currentView !== 'timeline' && (
