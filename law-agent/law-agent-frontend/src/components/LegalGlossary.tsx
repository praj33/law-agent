...
const LegalGlossary: React.FC<LegalGlossaryProps> = ({ terms, title }) => {
  ...
  const toggleBookmark = (term: string) => {
    const newBookmarked = new Set(bookmarkedTerms);
    if (newBookmarked.has(term)) {
      newBookmarked.delete(term);
    } else {
      newBookmarked.add(term);
    }
    setBookmarkedTerms(newBookmarked);
  };
  ...
  const exportToCSV = useCallback(() => {
    const headers = ['Term', 'Definition', 'Category', 'Importance', 'Examples', 'Related Terms'];
    const rows = displayTerms.map(term => [
      term.term,
      term.definition,
      term.category,
      term.importance || '',
      term.examples ? term.examples.join('; ') : '',
      term.relatedTerms ? term.relatedTerms.join('; ') : ''
    ]);
    ...
    link.setAttribute('download', 'legal_glossary.csv');
  ...
  const filteredAndBookmarkedTerms = useMemo(() => {
    return displayTerms.filter(term => bookmarkedTerms.has(term.term));
  }, [displayTerms, bookmarkedTerms]);
      ...
      >
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="flex items-center space-x-3">
            <button
              onClick={exportToCSV}
              className="flex items-center space-x-1 px-3 py-1.5 glass rounded-lg text-white text-sm hover:bg-white/10 transition-colors"
            >
            ...
            <div className="text-sm text-gray-400">
              ...
              {bookmarkedTerms.size > 0 && (
                <span className="ml-2 text-primary-400">• {bookmarkedTerms.size} bookmarked</span>
              )}
            </div>
          </div>
        </div>
        ...
        <div className="space-y-4">
          <AnimatePresence>
            {filteredTerms.map((term, index) => {
              ...
              const isBookmarked = bookmarkedTerms.has(term.term);
                  ...
                  >
                    <div className="flex items-center space-x-4 flex-1">
                      ...
                      <div className="flex-1">
                        <h3 className="text-lg font-semibold text-white mb-1 flex items-center">
                          {term.term}
                          {isBookmarked && (
                            <BookmarkCheck className="h-4 w-4 text-primary-400 ml-2" />
                          )}
                        </h3>
                        <div className="flex items-center space-x-3">
                    ...
                    <div className="flex items-center space-x-2">
                      <button
                        ...
                        className={`p-1 rounded hover:bg-white/10 transition-colors ${isBookmarked ? 'text-primary-400' : 'text-gray-400'}`}
                      >
                        ...
                        <div className="space-y-4">
                          ...
                          <div>
                            <div className="flex items-center justify-between mb-2">
                              <h4 className="text-sm font-medium text-white">Definition</h4>
                              <button
                                onClick={() => copyToClipboard(term.definition, term.term)}
